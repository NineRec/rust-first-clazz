...
// simply wrap these type for rust's orphan rule of trait
...
/// change SqlParser's offset expr to i64
...
mod tests {
    use super::*;
    use crate::dialect::QueryerDialect;
    use sqlparser::parser::Parser;
    use std::convert::TryInto;

    #[test]
    fn parse_sql_works() {
        let url = "http://httpbin.org/get?params=123";
        let sql = format!(
            "select a, b, c from {} where a=1 order by c desc limit 5 offset 10",
            url
        );

        let statement = &Parser::parse_sql(&QueryerDialect::default(), sql.as_ref()).unwrap()[0];
